...
func (s *Store) UpsertHNMTimerRecord(rec HNMTimerRecord) (HNMTimerRecord, error) {
	...
	const q = `
		INSERT INTO hnm_timers (
			id, guild_id, channel_id, hnm_id, last_kill, days_since_hq, is_notified, mod, created_at, updated_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	_, err := s.DB.Exec(q,
		...
		rec.Mod, toStrTime(rec.CreatedAt), toStrTime(rec.UpdatedAt),
	)
	return rec, err
}
...
func (s *Store) GetHNMTimerRecord(guildID, channelID, hnmID string) (HNMTimerRecord, bool, error) {
	const q = `
	SELECT id, guild_id, channel_id, hnm_id, last_kill, days_since_hq, is_notified, mod, created_at, updated_at
	FROM hnm_timers
	...
	err := s.DB.QueryRow(q, guildID, channelID, hnmID).Scan(
		&rec.ID, &rec.GuildID, &rec.ChannelID, &rec.HNMID,
		&lastKillStr, &rec.DaysSinceHQ, &isNotified, &rec.Mod,
