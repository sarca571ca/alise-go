...
- [x] Make the HNM Board look better by adding pictures etc.
- [x] Need to make checks when dealing with the Linkshell tables in the db for capitalization etc. to prevent duplicates.
- [x] Updating timers within the 20 minutes before camp will cause a 2nd channel to be created. Need to check if a channel is already made and use that one.
- [x] If the bot goes down need to reschedule any channels marked MoveScheduled in the HNMTimesCategory to be moved.
- [x] Manage grand wyrvn windows properly. Look at EnrageWindow usage from the enrage command.
- [x] Fix camp channels to be nicer
- [x] Timers dont seem to sort in order correctly on the hnm timer board
- [ ] ToD command that will automatically update the HNM Timer Board automatically
